...
#define TITLE_STRING  "%s|%s|%s|%s|%s|%s|%s|%s|%s|%s\n"
#define FORMAT_STRING "%s|%s|%s|%s|%s|%s|%s|%s|%ld|%ld\n"

	ast_cli(a->fd, "magnus\n");
	ast_cli(a->fd, "Channels with Call Detail Record (CDR) Information\n");
	ast_cli(a->fd, "--------------------------------------------------\n");
	ast_cli(a->fd, TITLE_STRING, "Channel", "sipaccount",  "number", "AccountCode","Status", "codec", "Dst. Channel", "LastApp", "Billsec", "Duration");

	/*
		channel              |sipaccount|number|   AccountCode|codec| Dst. Channel             |LastApp|Start    |Answer|End     |6415489|6339480
		SIP/sipproxy-00000000|CREDMARgd|019998485718|gdconsult|(g729)|SIP/Datora---CLI-00000002|Mbilling|10:37:33|      |10:37:35|0|2
	*/
	...
	for (; (cdr = ao2_iterator_next(&it_cdrs)); ao2_cleanup(cdr)) {
		...
		chan = ast_channel_get_by_name(it_cdr->party_a.snapshot->name);
		char codec[50];
		char chan_status[20];
		if (chan) {
			ast_channel_lock(chan);			
			snprintf(codec, sizeof(codec), "%s",ast_format_cap_get_names(ast_channel_nativeformats(chan), &codec_buf));	
			snprintf(chan_status, sizeof(chan_status), "%s",ast_state2str(ast_channel_state(chan)));		
		...
		ast_cli(a->fd, FORMAT_STRING, 
				it_cdr->party_a.snapshot->name,
				it_cdr->party_a.snapshot->peeraccount,
				it_cdr->party_a.snapshot->exten,
